...
bool WzExcel::deleteWorkSheet(const QString &sheetName)
{
    ...
    int count = workSheets->property("Count").toInt();
	workSheets->querySubObject("Item(int)", 1)->dynamicCall("delete");
  //  for(int i=1;i<=count;i++)
  //  {
		//
		///*if(workSheets->querySubObject("Item(int)",i)->property("Name").toString() == sheetName)
...
bool WzExcel::insertValue(const int &row, const int &column, const QString &value)
{
	...
	//居中
	data->setProperty("HorizontalAlignment", -4108);
	data->setProperty("VerticalAlignment", -4108);
    return true;
}
bool WzExcel::SetColumnWidth(const int &row, const int &column, const int &width) {
	data = workSheet->querySubObject("Cells(int,int)", row, column);
	data->setProperty("ColumnWidth", width);
	return true;
}
bool WzExcel::save()
{
...
void WzExcel::mergeCells(int topLeftRow, int topLeftColumn, int bottomRightRow, int bottomRightColumn)
{
	QString cell;
