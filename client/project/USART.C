...
/************函数功能：选择并配置串口属性********************
*************输入参数：UARTX哪个串口和*COMx相关属性**********
*************输出参数：无***********************************/
u8 byte_change(u8 Data)
{
	char i;
unsigned char Data1;
Data1 = Data;
...
{
    for (; *puts != 0;	puts++)  TX1_writebuff(*puts); 	//遇到停止符0结束
}
...
{
    for (; *puts != 0;	puts++)  TX2_writebuff(*puts); 	//遇到停止符0结束
}

/************函数功能：串口1发送字符串，发送到cnt个停止*****
*************输入参数：*puts，cnt***************************
*************输出参数：无***********************************/
